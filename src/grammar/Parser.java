...
public class Parser {
	...
	private TreeNode expression() {
		...
		if (token.isType(TokenType.ASSIGN)) {
			if (!t.nodeType.equals(NodeType.ARRAYVAR)
					&& !t.nodeType.equals(NodeType.VAR)) {
				IO.err(t.lineNo
						+ ": The left-hand side of an assignment must be a variable");
			}
			TreeNode p = new TreeNode();
			p.lineNo = token.getLineNo();
			p.setChild(0, t);
	...
	private void match(TokenType expected) {
		if (token.getType().equals(expected)) {
			...
			IO.err("ERROR:expect token " + token.getToken() + " at line "
					+ token.getLineNo());
		}
	}
